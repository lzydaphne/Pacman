...
Ghost* ghost_create(int flag) {
	...
	case Blinky:
		...
		ghost->move_script = &ghost_red_move_script; //20211222/1646 //有需要每個都寫一個程式來遊走嗎?
		break;
	case Pinky:
		ghost->objData.Coord.x = cage_grid_x;
		ghost->objData.Coord.y = cage_grid_y;
	...
	default:
		ghost->objData.Coord.x = cage_grid_x;
		ghost->objData.Coord.y = cage_grid_y;
		ghost->move_sprite = al_load_bitmap("Assets/ghost_move_red.png");
		ghost->move_script = &ghost_red_move_script;
		break;
	}
	return ghost;
...
void ghost_destory(Ghost* ghost) {
	...
	*/
	...
	al_destory_bitmap(ghost->flee_sprite);
	al_destory_bitmap(ghost->dead_sprite);
	free(ghost);*/
}
	...
	*/
	...
	if (ghost->status == FLEE) {
		...
		*/
		...
		{
		case RIGHT:
			al_draw_scaled_bitmap(ghost->move_sprite, 16 * 0, 0,
				16, 16,
				drawArea.x + fix_draw_pixel_offset_x, drawArea.y + fix_draw_pixel_offset_y,
				draw_region, draw_region,
				0
			...
			break;
		case LEFT:
			al_draw_scaled_bitmap(ghost->move_sprite, 16 * 2, 0,
				16, 16,
				drawArea.x + fix_draw_pixel_offset_x, drawArea.y + fix_draw_pixel_offset_y,
				draw_region, draw_region,
				0
			...
			break;
		case UP:
			al_draw_scaled_bitmap(ghost->move_sprite, 16 * 4, 0,
				16, 16,
				drawArea.x + fix_draw_pixel_offset_x, drawArea.y + fix_draw_pixel_offset_y,
				draw_region, draw_region,
				0
			...
			break;
		case DOWN:
			al_draw_scaled_bitmap(ghost->move_sprite, 16 * 6, 0,
				16, 16,
				drawArea.x + fix_draw_pixel_offset_x, drawArea.y + fix_draw_pixel_offset_y,
				draw_region, draw_region,
				0
