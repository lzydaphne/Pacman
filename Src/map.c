...
/* ----------------------------- start of default map code*/
	...
	if (filepath == NULL) {
		...
		game_log("%s\n", filepath);
		if (map1)
			pFile = fopen("Assets/map_nthu.txt","r"); //改過
		else if(map2)
			pFile = fopen("Assets/map_401.txt", "r"); //改過
		...
		if(fscanf( pFile , "%d %d", &(M->row_num), &(M->col_num)) != 2) {	//改過
	...
	*/
	...
	M->map = (char**)malloc(sizeof(char*) * M->row_num);//先挖1個column
	if (!M->map) {
		game_abort(stderr, "map char array malloc error\n");
		return NULL;
	}
	for (int i = 0; i < M->row_num; i++) {
		M->map[i] = (char*)malloc(sizeof(char) * (M->col_num));//再把row挖出來
		if(!M->map[i]){
			game_abort(stderr, "map char array malloc error\n");
			return NULL;
		}
	...
	*/
	...
	M->wallnum = M->beansCount = 0; //beasCount是豆總數
	for (int i = 0; i < M->row_num; i++) {
