...
#include "ghost.h"
#include "map.h"
#include "scene_GameOver.h" //added
#include <stdio.h> //added
extern bool animflag;
...
extern ALLEGRO_SAMPLE* start_music;

int game_main_Score = 0;
bool game_over = false;
bool game_win = false; // 2021 12 24為了吃完豆子設計的跳出遊戲的條件

/* Internal variables*/
static ALLEGRO_TIMER* power_up_timer;
static ALLEGRO_SAMPLE_ID START_MUSIC_ID;
...
static void init(void) {
	...
	if(!ghosts){
		...
		for (int i = 0; i < GHOST_NUM; i++) {
			...
			ghosts[i] = ghost_create(i);  //"BLINKY"
			if (!ghosts[i])
...
static void checkItem(void) {
	...
	switch (basic_map->map[Grid_y][Grid_x]) //吃到'.'的那一格位置
	{
	case '.':
		pacman_eatItem(pman, '.'); //吃掉後換成空格
		basic_map->beansCount += 1; //2021 12 24 計算pacman吃到的豆子總數
		game_main_Score += 10;
	default:
		break;
	}
	if (basic_map->beansCount == basic_map->beansNum)//2021 12 24 設計吃完豆子會贏的條件
		game_win = true;
...
static void update(void) {
	...
	if (game_over) {
		...
		*/
		...
		else if(al_get_timer_count(pman->death_anim_counter)>=18)
...
static void draw(void) {
	...
	//sprintf_s會檢查格式字串是否有有效的格式化字元，而sprintf只會檢查格式字串或緩衝區是否為Null指標。
	char buffer[50];
	sprintf_s(buffer,50, "SCORE = %d", game_main_Score);
	al_draw_text(
		menuFont,
...
static void render_init_screen(void) {
	...
	stop_bgm(START_MUSIC_ID);
	START_MUSIC_ID = play_audio(start_music, effect_volume);
